...
public:
	...
	void Insert(const std::string& key, T value)
	{
		...
		if (key.empty())
		{
			std::cout << "This map cannot store empty keys.\n";
			return;
		}
		...
		if (mData[hashIndex].key.empty())
		{
			mData[hashIndex].key = key;
			mData[hashIndex].value = value;
			mCurrentSize++;
			return;
		}
		std::cout << "Element cannot be inserted. Bucket at index: %s is full" << hashIndex;
